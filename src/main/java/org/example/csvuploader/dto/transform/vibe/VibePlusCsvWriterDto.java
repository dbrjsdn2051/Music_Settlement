...
@Setter
@NoArgsConstructor
public class VibePlusCsvWriterDto {
    ...
    private String companySongCode;           // 업체곡 코드
    ...
    public static VibePlusCsvWriterDto of(VibePlusCsvReadDto dto) throws IllegalAccessException {
        DateTimeFormatter yyyyMM = DateTimeFormatter.ofPattern("yyyyMM");
        YearMonth yearMonth = YearMonth.parse("20" + dto.getSettlementMonth().replaceAll("\"", ""), yyyyMM);
        YearMonth date = yearMonth.plusMonths(1);
        dto = SimpleQuoteRemover.removeSingleQuotes(dto);

        return VibePlusCsvWriterDto.builder()
                .settlementMonth(date.format(DateTimeFormatter.ofPattern("yyyyMM")))
